...
@Component
public class ImplemLeitorDocumento implements LeitorDocumento {
//public class LeitorDocumentoV1 {
    ...
    @Override
    public boolean iniciaLeitura(Object arq, Excessao excessao) {
        
        System.out.println("************************************* iniciaLeitura()");
        
        MultipartFile m = (MultipartFile) arq;
        this.configurarColunasParametros();
        ...
        try {        
            arquivo = Workbook.getWorkbook(m.getInputStream());            
            return true;
        } catch (IOException ex) {
            excessao = new Excessao(excessao, "[Problemas na leitura do arquivo]</br>");
            Logger.getLogger(ImplemLeitorDocumento.class.getName()).log(Level.SEVERE, null, ex);
            throw excessao;
    ...
    public int obterQuantidadeEnsaios() {
        return arquivo.getNumberOfSheets();
    }

    @Override
    public List obterValoresParametro(String classe, String parametro, int indexEnsaio) {
        //System.out.println("obterValoreParametro() classe: "+classe+" parametro: "+parametro+" indexEnsaio: "+indexEnsaio);
    ...
    private String obterCelulaString(int linha, int coluna, int planilha) {
        sheet = arquivo.getSheet(planilha);
        Cell ts;
        ts = sheet.getCell(coluna, linha);//coluna, linha
    ...
    private Double obterCelulaDouble(int linha, int coluna, int planilha) {
        ...
        Cell ts;
        ts = sheet.getCell(coluna, linha);//coluna, linha
        //if(ts.getContents() == "" || ts.getCellFormat() != NumberFormats.INTEGER) {
        if(ts.getContents() == "") {
            return null;
        }else{
            //return Double.valueOf(ts.getContents());
    ...
    private void configurarColunasParametros() {
        ...
        colParametrosLab.put("IndVaziosQualAmostra", 21);
        colParametrosLab.put("QualAmostra", 22);
        colParametrosLab.put("CoefAdensVertical", 23);        
        ...
        colParametrosCoefAdensamentoCPTU.put("U2_DPP_CPTU", 8);
        colParametrosCoefAdensamentoCPTU.put("Su(DPP)", 11);
    }
